  ...
  implements TypeValue
{
  ...
  public static ClassType coerceToClassTypeOrNull(Object paramObject)
  {
    ...
    if ((paramObject instanceof Class))
    {
      Type localType = Language.getDefaultLanguage().getTypeFor((Class)paramObject);
      if ((localType instanceof ClassType)) {
        return (ClassType)localType;
      }
    }
    return null;
  }
  ...
  public int compare(Type paramType)
  {
    ...
    for (;;)
    {
      i = getImplementationType().compare(paramType.getImplementationType());
      do
      {
        do
        {
          return i;
        } while ((paramType == typeType) || (paramType == typeClassType) || (paramType == typeType.implementationType));
  ...
  public Expression convertValue(Expression paramExpression)
  {
    ...
    ApplyExp localApplyExp = new ApplyExp(localMethod, new Expression[] { paramExpression });
  ...
  public void emitConvertFromPrimitive(Type paramType, CodeAttr paramCodeAttr)
  {
    ...
    while (str != null)
    {
      paramCodeAttr.emitInvokeStatic(ClassType.make(str).getDeclaredMethod("make", new Type[] { localObject }));
      return;
      boolean bool2 = paramType instanceof PrimType;
      localObject = null;
      str = null;
      if (bool2)
      {
        if ((paramType == Type.intType) || (paramType == Type.byteType) || (paramType == Type.shortType) || (paramType == Type.longType) || (paramType == Type.floatType))
        {
          paramCodeAttr.emitConvert(paramType, Type.doubleType);
          paramType = Type.doubleType;
        }
        PrimType localPrimType2 = Type.doubleType;
        localObject = null;
        str = null;
        if (paramType == localPrimType2)
        {
          str = "gnu.math.DFloNum";
          localObject = paramType;
          continue;
          boolean bool1 = paramType instanceof PrimType;
          localObject = null;
          str = null;
          if (bool1) {
            ...
            {
              str = "gnu.math.IntNum";
              localObject = Type.int_type;
            }
            else if (paramType == Type.longType)
            {
              str = "gnu.math.IntNum";
              localObject = Type.long_type;
            }
            ...
            else
            {
              ...
              PrimType localPrimType1 = Type.doubleType;
              localObject = null;
              str = null;
              if (paramType == localPrimType1)
              {
                str = "gnu.math.DFloNum";
    ...
    super.emitConvertFromPrimitive(paramType, paramCodeAttr);
  }
  ...
  public void emitTestIf(Variable paramVariable, Declaration paramDeclaration, Compilation paramCompilation)
  {
    ...
    Method localMethod = coercionOrNullMethod();
    if (localMethod != null) {
